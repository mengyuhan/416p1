...
func main() {
	...
	// // Blue Box
	shapeHash, blockHash, ink, err := canvas.AddShape(validateNum, blockartlib.PATH, "M 0 0 H 200 V 200 h -200 Z", "non-transparent", "red")
	if checkError(err) != nil {
		return
	}
	...
	// Add a line.
	shapeHash2, blockHash2, ink2, err := canvas.AddShape(validateNum, blockartlib.PATH, "M 500 500 L 50 0", "transparent", "blue")
	if checkError(err) != nil {
		return
	}
	...
	// ink3, err := canvas.DeleteShape(validateNum, shapeHash2)
	// if checkError(err) != nil {
	// 	return
	// }
	// fmt.Println(ink3)
